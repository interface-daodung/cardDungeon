...
class WarriorCardGame {
    constructor() {
        ...
        this.animationMode = 'dual'; // 'single' or 'dual'
    ...
    // Get appropriate move function based on animation mode
    getMoveFunction() {
        return this.animationMode === 'dual' ? this.moveWarriorDual.bind(this) : this.moveWarrior.bind(this);
    }
    ...
    // Handle warrior click - show hint
    handleWarriorClick(e, index) {
        console.log('Warrior clicked:', index);
        this.showHintForWarrior(index);
    }
    ...
    handleCardClick(e, index) {
        ...
        if (cardType === 'fatuice') {
            ...
            if (warriorIndex !== null && this.isValidMove(warriorIndex, index)) {
                console.log('Adjacent fatuice clicked:', index, 'Moving warrior from:', warriorIndex);
        ...
        // If clicked card is hint target (already highlighted), also move
            ...
            console.log('Hint target clicked:', index);
            const warriorIndex = this.findWarriorIndex();
            if (warriorIndex !== null && this.isValidMove(warriorIndex, index)) {
                this.getMoveFunction()(warriorIndex, index);
    ...
    // Show hint for specific warrior
    showHintForWarrior(warriorIndex) {
        this.clearValidTargets();
        
        let hasValidMoves = false;
        this.cards.forEach((card, index) => {
            if (this.isValidMove(warriorIndex, index)) {
        ...
        if (!hasValidMoves) {
            ...
            this.showMessage("Click on highlighted cards to move!");
        }
    }

    // Get all valid moves for a warrior
    getValidMoves(warriorIndex) {
        const validMoves = [];
    ...
    handleDrop(e) {
        ...
        console.log('Drop target:', targetIndex, 'Dragged:', this.draggedCard);
    ...
    handleTouchEnd(e) {
        ...
        if (targetCard) {
            const targetIndex = this.getCardIndexFromElement(targetCard);
            console.log('Touch end target:', targetIndex, 'Dragged:', this.draggedCard);
            if (targetIndex !== null && this.isValidMove(this.draggedCard, targetIndex)) {
                this.getMoveFunction()(this.draggedCard, targetIndex);
            }
        }
    ...
    isValidMove(fromIndex, toIndex) {
        ...
        console.log('Checking move:', fromPos, 'to', toPos);
        
        // Check if target is fatuice card
        if (this.cards[toIndex].type !== 'fatuice') {
            console.log('Target is not fatuice');
            return false;
        }
        ...
        console.log('Move valid:', isValid, 'rowDiff:', rowDiff, 'colDiff:', colDiff);
    ...
    // Calculate which cards need to move and where
    calculateCardMovements(fromIndex, toIndex) {
        const fromPos = { row: Math.floor(fromIndex / 3), col: fromIndex % 3 };
        const toPos = { row: Math.floor(toIndex / 3), col: toIndex % 3 };
        ...
        // Find cards that need to move
        this.cards.forEach((card, index) => {
            if (card && card.type === 'fatuice' && index !== toIndex) {
                const cardPos = { row: Math.floor(index / 3), col: index % 3 };
                ...
                if (shouldMove) {
                    const newIndex = this.calculateNewPosition(index, direction);
                    if (newIndex !== null) {
                        movements.push({ fromIndex: index, toIndex: newIndex, card });
                    }
                }
            }
        });
    ...
    // Find empty position in cards array
    findEmptyPosition(cards) {
    ...
    findCardToMove(fromIndex, toIndex) {
        ...
        console.log('Finding card to move. Direction:', direction, 'from', fromPos, 'to', toPos);
        ...
        for (let i = 0; i < this.cards.length; i++) {
            ...
            if (card && card.type === 'fatuice' && i !== toIndex) {
                ...
                if (shouldMove) {
                    console.log('Found card to move at index', i, 'position', cardPos);
                    return { card, fromIndex: i };
                }
            }
        }
        
        console.log('No card found to move');
        return null;
    }

    // Move warrior to eat fatuice
    moveWarrior(fromIndex, toIndex) {
        console.log('Moving warrior from', fromIndex, 'to', toIndex);
        ...
        // Animate the eating effect - fatuice shrinks and disappears
        fatuiceElement.classList.add('eating');
        
        // Animate warrior sliding to new position
        ...
        console.log('Animating warrior from', Boolean(cardToMove));
        // Animate the card that moves in reverse direction - ĐỒNG THỜI
        if (cardToMove) {
            ...
            if (reverseCardElement) {
                ...
                console.log('Animating reverse card from', cardToMove.fromIndex, 'to', fromIndex, 'with transform:', moveToX, moveToY);
                
                reverseCardElement.classList.add('reverse-moving-simultaneous');
                reverseCardElement.style.setProperty('--reverse-x', `${moveToX}%`);
                reverseCardElement.style.setProperty('--reverse-y', `${moveToY}%`);
                reverseCardElement.style.transform = `translate(${moveToX}%, ${moveToY}%)`;
            }
        } else {
            console.log('No reverse card to animate');
        }
        ...
        }, 250); // Start appear animation halfway through movement
    }

    // NEW: Enhanced dual movement animation
    moveWarriorDual(fromIndex, toIndex) {
        console.log('Dual moving warrior from', fromIndex, 'to', toIndex);
        const warriorCard = this.cards[fromIndex];
        const fatuiceCard = this.cards[toIndex];
        ...
        if (cardToMove) {
            ...
            if (reverseCardElement) {
                ...
                console.log('DUAL ANIMATION: Warrior and reverse card moving simultaneously');
            ...
            console.log('SINGLE ANIMATION: Only warrior moving');
            warriorElement.classList.add('sliding');
            warriorElement.style.setProperty('--move-x', `${moveX}%`);
            warriorElement.style.setProperty('--move-y', `${moveY}%`);
            warriorElement.style.transform = `translate(${moveX}%, ${moveY}%)`;
    ...
    // Calculate new position for a card based on movement direction
    calculateNewPosition(oldIndex, direction) {
        const oldPos = { row: Math.floor(oldIndex / 3), col: oldIndex % 3 };
    ...
    // Find the last empty position after all cards have moved
    findLastEmptyPosition(fromIndex, toIndex) {
        for (let i = 0; i < this.cards.length; i++) {
            if (this.cards[i] === null) {
                console.log('Found empty position at index:', i);
                return i;
            }
        }
